...
if("${insPortal.key}"=="GLOBAL-PERSONAL"){
	$(function(){
...
<style>
    ...
    {
        background: url("${ctxPath}/styles/images/index/contenticon.png") no-repeat 0px -95px; width: 40px; height: 15px; display: block; float: left; margin-right: 10px;
    }
    span.mini-tools-refresh {
    	background: url("${ctxPath}/styles/images/index/contenticon.png") no-repeat -82px -95px; width: 15px; height: 15px; display: block; float: left;margin-right: 10px;
    }
    
    span.mini-tools-collapse{
    	background: url("${ctxPath}/styles/images/index/contenticon.png") no-repeat -56px -95px; width: 15px; height: 15px; display: block; float: left;margin-right: 10px;
    }	
</style>
<style type="text/css">
a:link,a:visited {
...
<body>
	<div id="toolbar1" class="mini-toolbar" style="padding: 2px;">
		<table style="width: 100%;">
			<tr>
				...
				<span class="separator"></span>
				<a class="mini-button" iconCls="icon-clear" plain="true" onclick="cleanAll()">清空</a></td>
			</tr>
		</table>
	</div>
	<div>
    ...
    var pans =portal.getPanels();//获得所有的栏目数据
    ...
    function savePortal(){
		...
		//console.log(mini.encode(panels));
		_SubmitJson({
  ...
  //点击保存的函数
    function cleanAll(){
    	var portId = ${insPortal.portId};
		_SubmitJson({
			url : __rootPath + '/oa/info/insPortal/cleanAll.do?portId='+portId,
			method : 'POST',
			data : {portId:portId},
			success : function(result) {
				location.reload();
