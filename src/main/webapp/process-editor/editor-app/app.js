	                ...
	                jQuery(window).resize(function () {
	                    ...
	                    west.css('left', canvasLeft + iconOffset + 'px');
							...
							if($rootScope.editor){
								...
								var numPer=100*180/width;//固定180像素
								var paletteShouldBeWidth=numPer+"%";
								var canvasShouldBeWidth=100-numPer+"%";
								
								var display=jQuery("#paletteHelpWrapper").css('display');
								//若左边的值为隐藏
								if(display=='none'){
									paletteShouldBeWidth='0%';
									canvasShouldBeWidth='100%';
								}
								var propertiesShouldBeWidth=canvasShouldBeWidth;
								jQuery("#canvasHelpWrapper").width(canvasShouldBeWidth);
								jQuery("#canvasSection").width("100%");
								
								jQuery("#paletteHelpWrapper").width(paletteShouldBeWidth);
								jQuery("#propertiesHelpWrapper").width(propertiesShouldBeWidth);
								
								//canvas.setSize({width: width, height: height});
							}					
	                });
