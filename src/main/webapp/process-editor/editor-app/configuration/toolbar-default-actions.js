...
KISBPM.TOOLBAR = {
    ACTIONS: {
        ...
        expandToggle :function (services){
			var display=jQuery("#paletteHelpWrapper").css('display');
			if(display!='none'){
				jQuery("#paletteHelpWrapper").css('display','none');
			}else{
				jQuery("#paletteHelpWrapper").css('display','block');
			}
...
var SaveModelCtrl = [ '$rootScope', '$scope', '$http', '$route', '$location',
    ...
    $scope.modifyAndClose=function(){
    	$scope.save(function(){CloseWindow('ok');},'modify');
    };
    /**
