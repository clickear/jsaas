...
public class ${class}FormController extends BaseFormController {
     ...
     * @return
     */
    @ModelAttribute("${classVar}")
    public ${class} processForm(HttpServletRequest request) {
        String id = request.getParameter("id");
        ${class} ${classVar} = null;
        if (StringUtils.isNotEmpty(id)) {
            ${classVar} = ${classVar}Manager.get(id);
        } else {
            ${classVar} = new ${class}();
        }
    ...
    public JsonResult save(HttpServletRequest request, @ModelAttribute("${classVar}") @Valid ${class} ${classVar}, BindingResult result) {

        if (result.hasFieldErrors()) {
            return new JsonResult(false, getErrorMsg(result));
        }
        String msg = null;
        if (StringUtils.isEmpty(${classVar}.get${pkVar?cap_first}())) {
